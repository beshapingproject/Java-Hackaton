    ...
    return ResponseEntity.status(HttpStatus.OK)
            .body(
                    new JsonResponseBody(
                            HttpStatus.OK.value(), ticketService.getTicketList(customerId, filters)));
  }

  @PostMapping("/add/{customerId}")
  @ApiOperation(value = "", notes = "ADD NEW TICKET")
  public ResponseEntity<JsonResponseBody> addTicket(
          @ApiParam(name = "CUSTOMER", value = "Customer Id") @PathVariable(value = "customerId")
                  Long customerId,
    ...
    return ResponseEntity.status(HttpStatus.OK)
            .body(
                    new JsonResponseBody(
                            HttpStatus.OK.value(), ticketService.addTicket(customerId, ticket)));
  }
